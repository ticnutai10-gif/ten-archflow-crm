...
import { Meeting, Client, Project, User } from '@/entities/all';
...
import { format, startOfMonth, endOfMonth, eachDayOfInterval, isSameMonth, isToday, isSameDay, addMonths, subMonths, startOfWeek, endOfWeek, parseISO, addDays, startOfDay, endOfDay } from "date-fns";
...
export default function Meetings() {
  ...
  const handleStatusChange = async (meetingId, newStatus) => {
    try {
      await Meeting.update(meetingId, { status: newStatus });
  ...
  const handleSyncWithGoogle = async () => {
    if (!isGoogleConnected) {
      toast.error('יש להתחבר ל-Google תחילה בעמוד ההגדרות');
      return;
    }

    setIsSyncing(true);
    try {
      // כאן תוכל להוסיף קריאה ל-function שמסנכרן עם Google Calendar
      toast.info('הסנכרון עם Google Calendar יתווסף בקרוב');
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-slate-600">בוצעו</p>
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        {dayMeetings.slice(0, 2).map((meeting, idx) => (
                          <div
                            key={idx}
                            className={`text-[10px] px-1.5 py-0.5 rounded text-white truncate ${colorClasses[meeting.color]} flex items-center justify-between gap-1`}
                            onClick={(e) => {
                              e.stopPropagation();
                              handleEdit(meeting);
                            }}
                          >
                            ...
                            <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                              ...
                              >
                                <Bell className={`w-2.5 h-2.5 ${meeting.reminder_enabled ? 'fill-current' : ''}`} />
                              </button>
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        {dayMeetings.map((meeting) => (
                          <div
                            key={meeting.id}
                            className={`text-xs p-2 rounded cursor-pointer hover:shadow-md transition-all ${colorClasses[meeting.color]} text-white`}
