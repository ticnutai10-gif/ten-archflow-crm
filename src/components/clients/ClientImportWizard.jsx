  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (targetTable?.type === 'custom') {
        log(`📊 יבוא לטבלה מותאמת: "${targetTable.name}"`, 'info');
        ...
        log(`📦 יוצר ${newRows.length} שורות חדשות`, 'success');
        
        const currentTable = await base44.entities.CustomSpreadsheet.get(targetTable.id);
        const existingRows = currentTable?.rows_data || [];
        
        log(`📚 הטבלה מכילה ${existingRows.length} שורות קיימות`, 'info');
        
        const allRows = [...existingRows, ...newRows];
        
        log(`📊 סה"כ: ${allRows.length} שורות (${existingRows.length} + ${newRows.length})`, 'info');
        
        await base44.entities.CustomSpreadsheet.update(targetTable.id, { rows_data: allRows });
        ...
        if (verifyTable.rows_data?.length === allRows.length) {
          log(`✅ אימות הצליח: ${verifyTable.rows_data?.length} שורות`, 'success');
        } else {
          log(`⚠️ אזהרה: צפוי ${allRows.length}, נשמר ${verifyTable.rows_data?.length}`, 'warning');
        }
        
        successCount = newRows.length;
        log(`✅ ${successCount} שורות נשמרו לטבלה!`, 'success');
        
      } else {
        log(`📊 יבוא ${validatedData.length} לקוחות`, 'info');
        ...
        log(`✅ ${successCount} לקוחות יובאו`, 'success');
      }
      ...
      log(`❌ שגיאה קריטית: ${error.message}`, 'error');
